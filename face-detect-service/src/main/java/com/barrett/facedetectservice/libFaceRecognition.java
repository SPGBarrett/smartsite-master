...
public interface libFaceRecognition extends StdCallLibrary {
    ...
    String path = "E:\\smartSite\\sclib\\bin\\libFaceRecognitionSDK_x64";

    libFaceRecognition INSTANCE = (libFaceRecognition) Native.loadLibrary(path, libFaceRecognition.class);
     ...
     */
    void ZBX_RegFaceRecoCb(IntByReference cam, ZBX_FaceRecoCb_t cb, Pointer usrParam);
     ...
     * @brief 注册人脸抓拍数据接收回调函数
     */
    void ZBX_RegFaceOffLineRecoCb(IntByReference cam, ZBX_FaceRecoCb_t cb, Pointer usrParam);

    /**
     * @Param cam[in]  相机句柄
     * @Param squece[in]      需要查寻询的记录ID 从0开始
     * @Param face_info[out]  详见结构体
     * @return 无
     * @brief 注册人脸抓拍数据接收回调函数
     */
    int ZBX_QueryFaceOffLineReco(IntByReference cam, int squence, FaceRecoInfo face_info);

    /**************************************Some personalized Methods By Barrett 2020.10*********************************/
    /**
     * @Param cam[in]          相机句柄
     * @Param s_time[in]       开始时间 时间格式：2019-09-08 21:24:44
     * @Param e_time[in]       结束时间
     * @Param s_seq[out]      开始序号
     * @Param e_seq[out]      结束序号
     * @return 0/1
     * @brief 获取历史数据
     */
    int ZBX_GetResByTime(IntByReference cam, String startTime, String endTime, IntByReference startSeq, IntByReference endSeq);
