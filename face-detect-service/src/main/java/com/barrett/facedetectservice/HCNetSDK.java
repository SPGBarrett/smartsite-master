...
public interface HCNetSDK extends StdCallLibrary {
    ...
    // 一般情况，路径是基于jna.jar的路径的。但是在java后端程序中，需要将dll放置在resource的文件夹中，然后通过以下方式获取到路径。
    // 打包之后的文件会出现在target/classes中
    // For dev:
    //String path = (HCNetSDK.class.getResource("/hklib/").getPath()+"HCNetSDK.dll").replaceAll("%20", " ").substring(1).replace("/", "\\");

    //Using absolute path:
    //String path = "C:\\Users\\spg_b\\IdeaProjects\\smartsite-master\\face-detect-service\\target\\classes\\hklib\\HCNetSDK";
    // Path for deploy:
    String path = "E:\\smartSite\\hklib\\HCNetSDK";
...
//车牌识别（NET_VCA_PLATE_CFG）;
    ...
    public static final int NET_DVR_GET_FINGERPRINT_CFG = 2150;    //获取指纹参数
    public static final int NET_DVR_SET_FINGERPRINT_CFG = 2151;    //设置指纹参数
    ...
    public static final int NET_DVR_REMOTECONTROL_GATEWAY = 16009;  //远程开锁
...
//NET_DVR_Login()参数结构
    ...
    public static class NET_DVR_SETUPALARM_PARAM extends Structure {
        ...
        public byte byFaceAlarmDetection; //Bit0- 表示二级布防是否上传图片: 0-上传，1-不上传
    ...
    public static class NET_DVR_VEHICLE_INFO extends Structure {
        ...
        public byte[] byRes3 = new byte[16];
    }
    ...
    public static class NET_DVR_ACS_EVENT_INFO extends Structure {
        ...
        public byte byType;
        public byte[] byMACAddr = new byte[6]; // 物理地址，为0无效
    ...
    public static class NET_DVR_FINGER_PRINT_INFO_COND extends Structure {
        ...
        public byte[] byEnableCardReader = new byte[MAX_CARD_READER_NUM_512];  //指纹的读卡器信息，按数组表示
        public int dwFingerPrintNum; //设置或获取卡数量，获取时置为0xffffffff表示获取所有卡信息
        public byte byFingerPrintID;     //指纹编号，有效值范围为-10   0xff表示该卡所有指纹
        public byte byCallbackMode;     //设备回调方式，0-设备所有读卡器下完了范围，1-在时间段内下了部分也返回
        public byte[] byRes2 = new byte[2];
        public int dwEmployeeNo;        //工号（人员ID）
        public byte[] byRes1 = new byte[20];          //保留
    }
    ...
    public static class NET_DVR_FINGER_PRINT_CFG extends Structure {
        ...
        public byte[] byEnableCardReader = new byte[MAX_CARD_READER_NUM_512];  //需要下发指纹的读卡器，按数组表示，0-不下发该读卡器，1-下发到该读卡器
    ...
    public static class NET_DVR_FINGER_PRINT_STATUS extends Structure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //指纹关联的卡号
        public byte[] byCardReaderRecvStatus = new byte[MAX_CARD_READER_NUM_512];  //指纹读卡器状态，按字节表示，0-失败，1-成功，2-该指纹模组不在线，3-重试或指纹质量差，4-内存已满，5-已存在该指纹，6-已存在该指纹ID，7-非法指纹ID，8-该指纹模组无需配置
        public byte byFingerPrintID;     //指纹编号，有效值范围为1-10
        public byte byFingerType;       //指纹类型  0-普通指纹，1-胁迫指纹
        ...
        public int dwCardReaderNo;  //非0表示错误信息byErrMsg有效，其值代表byErrMsg对应的读卡器编号（具体什么错误查看byCardReaderRecvStatus对应编号的值）。0时表示无错误信息
        public int dwEmployeeNo;    //工号（人员ID）
        public int dwErrorEmployeeNo;   //下发错误信息，当byCardReaderRecvStatus为5时，表示已存在指纹对应的工号（人员ID）
        public byte[] byRes = new byte[16];
    }
    ...
    public static class NET_DVR_CID_ALARM extends Structure {
        ...
        public byte[] byRes2 = new byte[19];
    }
    ...
    public static class NET_DVR_ACS_CFG extends Structure {
        ...
        public byte[] byRes = new byte[501];
    }
    ...
    public static class NET_DVR_EVENT_CARD_LINKAGE_CFG_V51 extends Structure {
        public int dwSize;    //结构体大小
        public byte byProMode;                          //联动方式，0-事件，1-卡号
        public byte[] byRes1 = new byte[3];
        ...
        public byte byMainDevStopBuzzer;               //主机停止蜂鸣   0-不联动，1-联动输出
        public short wAudioDisplayID;                    //联动语音播放ID：0-不联动，目前范围1-32
        public byte byAudioDisplayMode;                 //联动语音播放模式：0-关闭，1-单次播放，2-循环播放
        public byte[] byRes3 = new byte[25];                         //保留
        ...
        public byte[] byReaderStopBuzzer = new byte[MAX_CARD_READER_NUM_512]; //联动读卡器停止蜂鸣，按字节表示，0-不联动，1-联动
    ...
    public static class NET_DVR_STRING_POINTER extends Structure {
        public byte[] byString = new byte[3 * 1024 * 1024];
    }
    ...
    public static final int NET_DVR_GET_ACS_WORK_STATUS_V50 = 2180;//MAX_CASE_SENSOR_NUM
    ...
    public static class NET_DVR_ACS_WORK_STATUS_V50 extends Structure {
        ...
        public int dwWhiteFaceNum; //已添加的白名单人脸数量（通过能力集判断）
        public int dwBlackFaceNum; //已添加的黑名单人脸数量（通过能力集判断）
...
//2007-11-30 V211支持以下接口 //11
    ...
    boolean NET_DVR_ContinuousShoot(NativeLong lUserID, NET_DVR_SNAPCFG lpInter);
    ...
    boolean NET_DVR_ControlGateway(NativeLong lUserID, int lGatewayIndex, int dwStaic);
    ...
    NativeLong NET_DVR_GetVehicleGpsInfo(NativeLong lUserID, NET_DVR_GET_GPS_DATA_PARAM lpGPSDataParam, fGPSDataCallback cbGPSDataCallback, Pointer pUser);
}
